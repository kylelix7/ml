...
## L3
 ...
 how to use segment register to avoid flush TLB when switching domain. steps when a call from one protection domain to another
  - use segment registers to store upper and lower bound of a region
  - assign fix size sub region to different domains
  - when switching to different domain, context switch is made to transfer control to the entry point in the called domain
  - ensure virtual addresses generated by the called domain are within the bounds of legal address for that domain
